...
`;
        ...
        // First check if model is in modelApiConfig for explicit mapping
        const modelConfig = appConfig.ai.modelApiConfig?.[model as keyof typeof appConfig.ai.modelApiConfig];

        let isAnthropic = model.startsWith('anthropic/');
        ...
        if (modelConfig) {
          const provider = (modelConfig as any).provider;
        ...
        // Use modelApiConfig if available for correct model name
        if (modelConfig) {
          actualModel = (modelConfig as any).model;
          console.log(`[generate-ai-code-stream] Using modelApiConfig: ${model} -> ${actualModel}`);
        } else if (isAnthropic) {
